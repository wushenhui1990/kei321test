...
#define TIME_OUT_GET_REPORT		20
#define TIME_OUT_SET_REPORT		20

#define	TRY_CNT					60					

#define TIMEOUT_REG_READ_WRITE	2000
    ...
    while(true)
    {
        if(GetMessage(&msg,0,0,0)) //get msg from message queue
        {
			...
			if(p_thread_recv_report)
			{
				...
				Sleep(TIME_OUT_GET_REPORT+20);
				p_thread_recv_report = NULL;
				pthis->m_work_style.SetWindowText(_T("idle"));
			}
			...
			TRACE("cnt =%d\n",cnt);
			if((msg.message == MSG_SWITCH_WORK_STYLE) && 
				( pInfo[0] == WORK_STYLE_PREVIEW) && 
				(p_thread_recv_report==NULL))
			{
				p_thread_recv_report = AfxBeginThread(receive_report_thread,pthis,0,0,0,0);
				pthis->m_work_style.SetWindowText(_T("preview"));
			}
...
unsigned int receive_report_thread(void *param)
{
	...
	while (1)
	{
		...
		//WaitForSingleObject(g_event_reg_rdwr.m_hObject,INFINITE);
...
void CTPA06CA_AdjusterDlg::OpenUsb()
{
		...
		if (results != 0)
		{
			...
			if (results == HID_DEVICE_SUCCESS)
			{
				...
				HID_Blinky.SetTimeouts (TIME_OUT_GET_REPORT,TIME_OUT_SET_REPORT);

				if(p_thread_recv_report == NULL)
				{
					p_thread_recv_report = AfxBeginThread(receive_report_thread,this,0,0,0,0);
				}
...
void CTPA06CA_AdjusterDlg::CloseUsb()
{
	...
	if (HID_Blinky.IsOpened())
	{
		if (HID_Blinky.Close() == HID_DEVICE_SUCCESS)
		{
			...
			m_work_style.SetWindowTextW(_T("idle"));
...
//check if COM opened
	...
	else
	{
		if (NULL == p_RunScriptThread)
		{
				...
				{
					g_cmd_buff[0] = 0;
					PostThreadMessage(p_thread_main_work->m_nThreadID,MSG_SWITCH_WORK_STYLE,(WPARAM)g_cmd_buff,0);//post thread msg
